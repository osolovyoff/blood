...
function(configuration_linking_flags)
	...
	if (UNIX)
		if(RASPBERRY)
			...
			set(CMAKE_CXX_FLAGS "${CMAKE_C_FLAGS} ${CMAKE_CXX_FLAGS} -fpermissive -std=c++11 -fPIC" PARENT_SCOPE)
			set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} -static -O2 -lm -mcpu=arm1176jzf-s -mfpu=vfp -mfloat-abi=hard" PARENT_SCOPE)
			set(CMAKE_C++_CREATE_STATIC_LIBRARY TRUE PARENT_SCOPE)	
		else()
			set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -fpermissive -std=c++11 -lm -fPIC" PARENT_SCOPE)
...
function(configuration_project_types)
	...
	if (WIN32)
	    set(BLOOD_PLATFORM ${BLOOD_PLATFORM} WIN32)
	endif()
	if (MINGW)
	    set(BLOOD_PLATFORM ${BLOOD_PLATFORM} MINGW)
	endif()
	if (SYGWIN)
	    set(BLOOD_PLATFORM ${BLOOD_PLATFORM} SYGWIN)
	endif()
	if (MSVC)
	    set(BLOOD_PLATFORM ${BLOOD_PLATFORM} MSVS)
	    if (STORE)
	    	set(BLOOD_STORE ${BLOOD_PLATFROM} STORE)
	    endif()
	endif()
	if (UNIX)
	    set(BLOOD_PLATFORM ${BLOOD_PLATFORM} UNIX)
	endif()
	foreach(PLATFORM ${BLOOD_PLATFORM})
